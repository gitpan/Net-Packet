...
our @ISA = qw(Net::Packet::Layer3 Exporter);
our @EXPORT_OK = qw(
   NETPKT_IPv4_HDR_LEN
   NETPKT_IPv4_V4
   NETPKT_IPv4_PROTOCOL_TCP
   NETPKT_IPv4_PROTOCOL_UDP
   NETPKT_IPv4_PROTOCOL_ICMPv4
...
use constant NETPKT_IPv4_PROTOCOL_ICMPv4   => 1;
use constant NETPKT_IPv4_PROTOCOL_TCP      => 6;
use constant NETPKT_IPv4_PROTOCOL_UDP      => 17;
...
sub encapsulate {
   my $types = {
      NETPKT_IPv4_PROTOCOL_TCP()    => Net::Packet::Frame::NETPKT_LAYER_TCP(),
      NETPKT_IPv4_PROTOCOL_UDP()    => Net::Packet::Frame::NETPKT_LAYER_UDP(),
      NETPKT_IPv4_PROTOCOL_ICMPv4() => Net::Packet::Frame::NETPKT_LAYER_ICMPv4(),
   };

   $types->{shift->protocol} || Net::Packet::Frame::NETPKT_LAYER_UNKNOWN();
}
...
sub isProtocolTcp    { shift->protocol == NETPKT_IPv4_PROTOCOL_TCP    ? 1 : 0 }
sub isProtocolUdp    { shift->protocol == NETPKT_IPv4_PROTOCOL_UDP    ? 1 : 0 }
sub isProtocolIcmpv4 { shift->protocol == NETPKT_IPv4_PROTOCOL_ICMPv4 ? 1 : 0 }

1;

__END__
   
=head1 RFC 791 - Internet Protocol
